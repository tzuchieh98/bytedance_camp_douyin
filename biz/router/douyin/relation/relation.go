...
import (
	...
	relation "github.com/linzijie1998/bytedance_camp_douyin/biz/handler/douyin/relation"
)
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_message := _douyin.Group("/message", _messageMw()...)
			{
				...
				_action.POST("/", append(_message_ctionMw(), relation.MessageAction)...)
			}
			{
				_chat := _message.Group("/chat", _chatMw()...)
				_chat.GET("/", append(_messagechatMw(), relation.MessageChat)...)
			}
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action0.POST("/", append(_relation_ctionMw(), relation.RelationAction)...)
			}
			{
